...
{
    public partial class SentryBoxList : BaseWindow
    {
        public SentryBoxList()
        {
            InitializeComponent();
        }
        ...
        private void InitData()
        {
            ...
            DataTable dt = MySQLHelper.ExecuteQuery("select s.*,p.`name` as parkingname from t_sentrybox s left join t_parking p on s.parkingid=p.id").Tables[0];
            foreach (DataRow dr in dt.Rows)
            {
                ...
                row.Cells["colEnterInfo"].Value = dr["enterinfo"];
                row.Cells["colOutInfo"].Value = dr["outinfo"];
                row.Cells["colVideo1"].Value = dr["video1"];
                row.Cells["colVideo2"].Value = dr["video2"];
                row.Cells["colVideo3"].Value = dr["video3"];
                row.Cells["colVideo4"].Value = dr["video4"];
            }
        }
        #endregion
        ...
        private void cmdEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                AddSentryBox frmEdit = new AddSentryBox(ID);
        ...
        {
            AddSentryBox frmAdd = new AddSentryBox();
        ...
        private void cmdDel_Click(object sender, EventArgs e)
        {
            ...
            {
                string sql = string.Format("delete from t_sentrybox where id in ({0})", args);
