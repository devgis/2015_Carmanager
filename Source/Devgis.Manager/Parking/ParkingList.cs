...
{
    public partial class ParkingList : BaseWindow
    {
        public ParkingList()
        {
            InitializeComponent();
        ...
        private void InitData()
        {
            ...
            DataTable dt = MySQLHelper.ExecuteQuery("select * from t_parking").Tables[0];
            foreach (DataRow dr in dt.Rows)
            {
                GridViewRowInfo row = rgvDataList.Rows.AddNew();
                row.Cells["colID"].Value = dr["id"];
                row.Cells["colName"].Value = dr["name"];
                row.Cells["colAddress"].Value = dr["address"];
                row.Cells["colX"].Value = dr["x"];
                row.Cells["colY"].Value = dr["y"];
                row.Cells["colBalance"].Value = dr["balance"];
                row.Cells["colZone"].Value = dr["zone"];
                row.Cells["colSpaceCount"].Value = dr["spacecount"];
            }
        }
        #endregion
        ...
        {
            AddParking frmAddOperator = new AddParking();
