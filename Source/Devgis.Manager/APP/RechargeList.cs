...
{
    public partial class RechargeList : BaseWindow
    {
        public RechargeList()
        {
            InitializeComponent();
        ...
        private void InitData()
        {
            ...
            DataTable dt = MySQLHelper.ExecuteQuery("select r.id,m.realname as membername,o.realname as operatorname,r.amount,r.rechargetime,r.remarks from t_rechargehistory r left join t_member m on r.memberid=m.id left join t_operator o on r.operatorid=o.id").Tables[0];
            foreach (DataRow dr in dt.Rows)
            {
                ...
                row.Cells["colTime"].Value = dr["rechargetime"];
        ...
        private void cmdEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                AddMember frmEdit = new AddMember(ID);
        ...
        {
            AddRecharge frmAddRecharge = new AddRecharge();
            if (frmAddRecharge.ShowDialog() == DialogResult.OK)
            {
                //刷新
                InitData();
            }
