...
{
    public partial class MemberList : BaseWindow
    {
        public MemberList()
        {
            InitializeComponent();
        ...
        private void InitData()
        {
            ...
            DataTable dt = MySQLHelper.ExecuteQuery("select * from t_member where isdeleted <>1 ||isdeleted is NULL").Tables[0];
            foreach (DataRow dr in dt.Rows)
            {
                ...
                row.Cells["colIDCard"].Value = dr["idcard"];
                row.Cells["colBalance"].Value = dr["balance"];
                row.Cells["colMobile"].Value = dr["mobile"];
            }
        }
        #endregion
        ...
        {
            AddMember frmAddOperator = new AddMember();
