...
{
    public partial class AddMember : BaseWindow
    {
        ...
        private bool Check()
        {
            ...
            if (string.IsNullOrEmpty(rtbMobile.Text.Trim()))
            {
                MessageHelper.ShowErrorMessage("请输入手机号！");
                rtbMobile.Focus();
                return false;
            }
        ...
        private bool Save()
        {
            ...
            if (string.IsNullOrEmpty(EditID))
            {
                ...
                sql = "insert into t_member (id,loginname,realname,password,idcard,isdeleted,balance,mobile)values(@id,@loginname,@realname,@password,@idcard,0,0,@mobile)";
                ...
                arrParameter[5] = new MySqlParameter("@mobile", rtbMobile.Text);
            ...
            else
            {
                //修改
                MySqlParameter[] arrParameter = new MySqlParameter[6];
                sql = "update t_member set loginname=@loginname,realname=@realname,password=@password,idcard=@idcard,isdeleted=0,mobile=@mobile where id=@id";
